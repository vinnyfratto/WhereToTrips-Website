...
- **2026-09-20** — **Copy edit pass over everything written today.** Swept eight pages plus the three generators. **American English:** "queue through" → "line up for" (Chiang Mai's card hook, in `gen-featured-vibes.js`), "snorkeller" → "snorkeler", "travellers" → "travelers". **AI tropes: none found** — no "delve", "tapestry", "seamless", "isn't just X it's Y", "whether you're", none of it; and **no em-dashes in body prose** on any of the eight (the only ones on the site are `<title>` separators, which are the existing convention). The real problem was different: the new copy avoided contractions almost everywhere, which reads stiff and is its own kind of tell, and the house voice uses them. Introduced where they help the rhythm, not mechanically. Trimmed roughly 10% off the longer sections — mostly the Naples lede, the 690-to-147 explanation and the four gates. **Caught during the sweep:** the Travel Vibes stat note still said "the shelves those vibes sit on", which is the internal word we stopped using publicly. `package.json` 1.33.0 → 1.33.1.

- **2026-09-20** — **`/travel-vibes/` and `/destinations/` rewritten.** Both now argue their case with live engine data instead of stock copy. `engine_stats` gains a **`set`** picker so each page prints its own four numbers: Vibes shows 12,000+ matches / 690+ styles scored / **147 you pick from** / 24 collections, and Destinations shows 1,000+ bookable / **155 countries** / 1,100+ gateways / 8,000+ places scored around them (new `distinctCount` helper, since PostgREST has no COUNT DISTINCT). **Travel Vibes** explains the six scoring measures in order of weight and, plainly, why 690 styles collapse to 147: nobody scrolls 690 of anything, so near-twins merge and the short list still points at the long one. Photos are shelf images and the page closes on all 24 collections. **Destinations** covers the four gates a place must clear (measured hotel inventory, real US passenger traffic rather than nearest-runway distance, current State Dept advisories, and whether it is actually good at anything) and then a new **`gateway_example`** section opens one airport up: **NAP → 27 places, 35 vibes, 45 scored rows**, listing Pompeii & Herculaneum, the Amalfi Coast, Capri, Vesuvius, Ischia and Paestum with their real scores. Those are live rows, not an illustration. `package.json` 1.32.0 → 1.33.0. **Caught during the rewrite:** dropping the old hub's `collection_grid` orphaned the four curated destination lists — only the sitemap still pointed at them. A "Curated lists" grid is back on `/destinations/` to keep them linked.

- **2026-09-20** — **The four workflow pages rewritten off today's copy, with shelf photography.** `/whereto/discover/`, `/whereto/explore/`, `/whereto/direct/` and `/wander/together/` are each now hero → the actual steps of that workflow → one honest "what you get" split (Direct skips it; its workflow is three steps and the page should be three steps) → CTA. **No `/media/` stock left on any of them** — every photo is a `vibe_shelves` image, and Discover's hero cycles the ten categories, which is literally what its first screen asks you to do. `feature_split` and the plain `hero` `<img>` now run through `thumb()`/`thumbSet()` too, so putting remote photos in them could not reintroduce the oversized-image problem; local `/media/` paths pass through untouched so every other page is unaffected. `package.json` 1.31.0 → 1.32.0. **Open question for the owner:** `/whereto/direct/` exists and is linked from `/whereto/` and `/how-it-works/`, but is NOT in the nav dropdown (which lists only Discover and Explore). Either add Direct and Wander Together to that dropdown so the nav matches the "four ways in" story, or drop the page and its two links.

- **2026-09-20** — **srcset on every cycling surface.** Phones were downloading the desktop sizes for tiles they render at ~160px. New **`thumbSet(widths)`** filter beside `thumb()`, plus a `sizes` attribute on each surface (without `sizes` the browser assumes 100vw and picks the largest anyway). Deferred frames carry **`data-srcset`** alongside `data-src`, and `hydrate()` sets srcset FIRST — setting `src` alone starts a fetch of the fallback that the responsive candidate then overrides. Measured at 375px/dpr2: hero 1100w → **480w**, tiles 560w → **360w**. `package.json` 1.30.1 → 1.31.0.

- **2026-09-20** — **The How It Works hero photo was invisible on every phone.** `.hero-grid` carries `align-items: center`, and at ≤960px it becomes a flex **column**, so its children shrink-to-fit rather than stretching. A plain `<img>` survives that on its intrinsic width, which is why every other hero on the site is fine. The cycling hero is a `<span>` whose only children are absolutely positioned, so it has **no intrinsic width at all** and resolved to **0px wide** — perfect on desktop, blank on mobile. Added a `hero-media-cycle` modifier with `align-self: stretch`. Measured at 375px: 0×260 → 339×260. `package.json` 1.30.0 → 1.30.1. **Lesson:** swapping an `<img>` for a wrapper element removes intrinsic sizing, and any shrink-to-fit ancestor then collapses it. Check every breakpoint after that kind of swap, not just the one you are looking at.

- **2026-09-20** — **The real reason the images looked broken: the site was serving full-size originals into thumbnails.** The category and collection banners are 1600-1800px and the destination stock shots larger again; `/how-it-works/` was pulling ~100 of them into 230px boxes, so tiles sat white for ten seconds or more. Added a **`thumb(width, quality)`** Eleventy filter using **weserv** (never Supabase's own transform endpoint — the same rule the app follows in `src/utils/imageSource.ts`), and every cycling surface now requests at roughly 2x its display size: hero 1100, mode and collection tiles 560, destination cards and the gallery 760. A category banner drops from 243 KB to 128 KB at 1100, and far more than that at 560. Non-http values (local `/media/` paths) pass through untouched. `package.json` 1.29.3 → 1.30.0.

- **2026-09-20** — **Third and last part of the image fix: the crossfade blinked white.** Both frames were transitioning opacity at once (outgoing 1→0 while incoming 0→1), so at the midpoint each sat at 0.5, the stack composited to half-transparent, and the tile's grey placeholder showed through. One visible blink per swap, on 29 cycling boxes, which is indistinguishable from an image failing to load. The new frame now fades in **on top** (incrementing z-index) and the outgoing one is only switched off once the incoming is fully opaque. `package.json` 1.29.2 → 1.29.3.

- **2026-09-20** — **Second half of the image fix: the FIRST frame was lazy too.** The `data-src` change fixed the deferred frames, but every tile's first frame — the one that IS the content — still shipped `loading="lazy"`, so on a real browser you scrolled to a tile and got a white box for a beat before the photo arrived. Verified in actual Chrome (the desktop Browser pane never paints, never fires IntersectionObserver and never decodes images, so it cannot catch this class of bug at all — do not trust it for anything visual). First frames are now `eager` across the cycling surfaces; only the genuinely deferred frames stay out of the initial load, via `data-src`. The featured-vibe photo gallery stays fully lazy, since it is decoration far down a long page. Also: `.stat-num` was breaking "2.8 million+" after "2.8" in a quarter-width column, so it reads as two numbers — resized and set `nowrap`. `package.json` 1.29.1 → 1.29.2.

- **2026-09-20** — **Fixed the cycling photos: nothing past frame one was ever loading.** Every stacked frame shipped as `loading="lazy"`, and a lazy image sitting at `opacity: 0` underneath an opaque sibling is never "needed", so the browser never fetched it, so `complete` stayed false, so `next()` skipped it — permanently. Every cycling surface on the site was stuck on its first photo: the featured vibe cards, the four mode tiles, the 24 collections, the detail-page heroes and the new category hero. Now only the first frame carries `src`; the rest carry **`data-src`** and are hydrated by `wt-vibe-cards.js`. **Hydration runs off the stagger TIMER, not the IntersectionObserver** — an observer that never fires (backgrounded tab, throttling; the desktop Browser pane does exactly this) would reintroduce the same bug, so the observer is now a pure pause-when-offscreen optimisation and is allowed to never fire. Also removed the "Browse the vibes" ghost button from the How It Works hero. `package.json` 1.29.0 → 1.29.1.

- **2026-09-20** — **"Shelves" is now "collections" on the public site, and the How It Works hero cycles the ten categories.** `hero.njk` gains an `imageCycle: "categories"` mode that stacks `vibe_categories.image_1` for all ten and crossfades them through `wt-vibe-cards.js`, instead of a single still. The section type `shelf_mosaic` → **`vibe_collections`**, its CSS classes `.shelf-*` → `.collection-*`, and the copy, CMS label and stat note follow. **Why not "vibe groups":** "group" already means a group of travellers in Wander Together, and that word has to keep meaning one thing. "Shelf" stays the internal word everywhere it already is — `vibe_shelves` in Supabase, `shelves` in the Vibes Engine content tree, and the three internal image tools — so nothing in the app or the engine is renamed. `package.json` 1.28.1 → 1.29.0.

- **2026-09-20** — **Early Access removed entirely, not just relabelled.** The CTA rename left the phrase in the FAQ, the admin dashboard tile, the submissions page description and two JS headers; all gone. The FAQ's "Is WhereTo available now?" answer was also shipping an internal **"PRODUCTION NOTE: update per current launch stage..."** straight to the public, now a real answer (pre-launch, October 2026, Notify Me at Launch). **/how-it-works/** hero photo is now a **shelf image** (`vibe_shelves.mountains_scenic_landscapes.image_1`) instead of the laptop stock shot — the shelf images are 1800x580 banners and this one crops well to the hero's tall box. **"Researched by hand" / "written by hand" is out of all copy**, both in the page body and in the stat note the generator writes. `package.json` 1.28.0 → 1.28.1. **Still open:** `/about/` ships its own public "PRODUCTION NOTE" about founder narrative — left alone because replacing it means inventing founder history, not editing copy.

- **2026-09-20** — **/how-it-works/ rebuilt, home page trimmed, Early Access retired.** The orphan `src/_includes/base.njk` (402 dead lines from the layouts/ refactor) is deleted. **How It Works** is now a real page: hero, the `app_modes` **Four ways in** block with the full workflow descriptors, a new **`engine_stats`** section, a `steps` block on what the engine weighs, and a new **`shelf_mosaic`** showing all **24 vibe shelves with the same photos the app's picker uses** (`vibe_shelves.image_1..3`, three per shelf, cycled and staggered). Its old page described "Six modules" including the removed Chat and called the engine an "AI Discovery Engine" in its meta description, which breaks the house rule — both gone. **Every stat is counted, never typed**: `scripts/gen-engine-stats.js` writes `src/_data/engineStats.json` with `display` (rounded **down**, always with a "+") beside `exact`, so the rounding can be checked. Today: **12,000+** scored vibe matches (12,936), **1,000+** destinations (1,025), **2.8 million+** words across 2,363 guides (2,801,773), **690+** travel styles (692) grouped into 147 across 24 shelves. **Home page:** "See it in action" (`video_banner`), the reviews (`testimonial`) and "From the blog" (`collection_grid`) are all removed; neither of the first two is used anywhere on the site any more, though their partials and CMS entries are kept for launch. **Site-wide:** "Join Early Access" is gone — `cta_band`'s default label is now "Notify me on launch", its Web3Forms subject and `intent` move from `early_access` to **`notify_launch`** so those submissions land with the dedicated page's rather than in a second list, and the footer and FAQ copy follow. `package.json` 1.27.1 → 1.28.0.

- **2026-09-20** — **WhereTo: Chat cleared out of the site.** The feature was removed from the app on purpose and the website still had a live leaf page for it. Deleted `src/whereto/chat.md` and the now-unused `sections/chat.njk`, its CSS block, and its CMS palette entry. The two module pickers that linked to it were fixed rather than just shortened: `/whereto/` still says "Four ways" so **Wander Together** takes the fourth tile (matching the home page's new "Four ways in"), and `/how-it-works/` goes **"Six modules" → "Five modules"**. `/media/chat.jpg` stays — it is a generic hero photo on seven unrelated pages, not the feature. Also reworded the no-ads section's lede, fourth point and closing line. `package.json` 1.27.0 → 1.27.1. **Still open:** `src/_includes/base.njk` (402 lines) is an orphan from the layouts/ refactor — nothing references it, and it carries its own stale copy of the chat CSS.

- **2026-09-20** — **"Conversational Search" replaced with "No ads."** The old `chat` section was also selling a feature that was removed on purpose (see the app's Removed Chat note), so it was advertising something nobody can use. New `no_ads` section (`src/_includes/sections/no_ads.njk`): no ad slot exists, what you tell us is not sold, nobody buys their way up the results, and an honest fourth point that we DO collect device and usage data for diagnostics. **Every claim is pinned to the signed Privacy Statement** — "does not sell personal information" and "we do not share ... for their own independent marketing purposes" are Section 4 verbatim; the diagnostics point is Section 2's device-and-usage row. The partial and the CMS field both carry a warning not to add a claim the policy does not already make, because this is the one section on the site where marketing copy can create legal exposure. No boxes around the points (house default). `package.json` 1.26.1 → 1.27.0. **Still open:** `src/whereto/chat.md` and the `chat` section partial are still in the repo and still in the nav data; the removed feature has a live leaf page.

- **2026-09-20** — **"Wander Family Teaser" replaced with "Four ways in."** The old `feature_split` sold Wander Together on its own next to one stock group photo, which left the other three ways to use the app unmentioned on the home page. New `app_modes` section (`src/_includes/sections/app_modes.njk`): the intro copy on the left, and on the right a **2x2 of the app's own dashboard tiles** — a port of `app/index.tsx`'s `PhotoTile` down to the 20px-radius white card, the photo at ~62% of the tile, the short azure rule, the amber double-arrow, and the title with its **last word in azure bold italic**. Each tile cycles the SAME `Dashboard_Images-*` photos the phone rotates (Discover / BudgetVibes for Explore / Direct / WanderTogether), crossfading through `wt-vibe-cards.js` with a per-tile stagger, and links to that mode's existing leaf page. Under the split, the four longer explanations. Photos come from a committed `src/_data/appModes.json` written by `node scripts/gen-app-modes.js` — same committed-snapshot rule as `gen-featured-vibes.js`. The tile blurb is clamped to three lines exactly as the app caps it, which also keeps the photo at its 60/40 share; the full text sits right below. `app_modes` registered in the CMS palette (copy editable, tiles generated). `package.json` 1.26.0 → 1.26.1. **Gotcha for next time:** `index.md` front matter is YAML, so a lede containing `": "` has to be quoted or the whole build dies on "bad indentation of a mapping entry".

- **2026-09-20** — Dropped "The Vibe Engine sends you here." from the featured-vibe detail hero. The line now reads just "Most people book &lt;obvious place&gt;." `package.json` 1.24.1 → 1.24.2.

- **2026-09-20** — **Bonaire replaces Oaxaca.** The swap has to be like for like and Oaxaca-against-Cancún was not: one is a beach week, the other is craft villages in the hills, and nobody choosing between them is choosing between them. **Bonaire against Aruba** is a real pair — same ABC chain, same dry sun, same hurricane-belt exemption, same flight corridor, a beach trip either way — and Bonaire's Reef Snorkeling is the S-tier top of the Caribbean pass at 90 (shore-entry marine park, 11 ranked vibes). Also: `placeLabel` is now nulled when it would repeat the destination's own name, so an island that is its own country doesn't print "Bonaire · Bonaire"; the card, hero, title and meta description all handle it being absent. `package.json` 1.24.0 → 1.24.1.

- **2026-09-20** — **Crete did not need a scoring pass; the generator was reading the wrong column.** `gen-featured-vibes.js` counted a destination's vibes through `vibe_destination_rankings.dest_id`, which reported two rows for Heraklion and got Crete dropped from the six. `dest_id` is the **editorial** link — "this row names a place we sell individually" — and it is null for **8,338 of the 12,936** ranking rows *by design* (see `supabase/migrations/20260828_vibe_vocabulary_part3.sql`, which spells this out): the rankings are a corpus of places you reach from a gateway, not a list of gateways, so rows like "The Diktean cave" and "Cretan raki & tsikoudia" are correctly unlinked. The app matches on **`iata`** (`src/lib/vibeRankings.ts`) and the generator does now too, deduped to one row per vibe. Every count jumped: Heraklion 2 → **19**, Quebec City 8 → 25, Savannah 12 → 23, Oaxaca 7 → 16, Chiang Mai 9 → 12, San Sebastián 7 → 9. **Crete is back in the six** (Thessaloniki out), carded as "Crete" while the gateway stays Heraklion/HER — the fact strip shows both. The vibe list is retitled "Every vibe &lt;place&gt; **reaches**" and prints each row's own place name, so "The Heraklion Archaeological Museum" is not misattributed to the gateway. Also dropped the "Researched by hand · last updated" byline. `package.json` 1.23.1 → 1.24.0. **Nothing in the Vibe Engine was changed** — no ranking rows written, so no `sync:masters` owed.

- **2026-09-20** — Featured vibe card pass 3. **Picks re-cut for engine depth**: the Explore page's "every vibe this place matches" list is the proof the engine is real, and Heraklion holds exactly **two** ranking rows (a genuine coverage gap in the engine, not a rendering bug — it is the only Cretan gateway scored at all). Thessaloniki takes the Greek slot (nine vibes, same "not the postcard island" story, and the Vergina gold really is there rather than in Athens); **Savannah, Georgia** replaces Kanazawa (four vibes) as the required US pick. The generator now **refuses** any pick under `MIN_VIBES = 6`. Cards carry a country + sub-region line, and a US card prints its **state**, never "United States" — the engine has no state column (`admin_area` reads "United States" for nearly every US row), so `PICKS[].state` is a hand-checked per-pick value; the line collapses to one term when they'd repeat (Oaxaca's sub-region is "Mexico"). Detail page: every section heading is centered while its prose stays left on a 760px measure (the intro was centered like a pull-quote), and the guide is now titled for the **sub-region** it actually covers — "The vibe guide for the Mediterranean" / "The Mediterranean at its best", with the blog's own title demoted to the lede. Article handling for `Mediterranean` and the `US *` sub-regions. `package.json` 1.23.0 → 1.23.1.

- **2026-09-20** — Featured vibe card pass 2: dropped the dashed azure gateway roundel from the photo's top-right corner, collapsed the two-button stack down to a single **Discover &lt;City&gt;** CTA that is the site's own `.btn .btn-primary` (no card-local button variant, so its hover is the page's hover), and fixed the detail page's "Best window" — it was `truncate(4)` on full month names, printing `Janu · Marc`, now a real three-letter table (`Nov · Dec · Jan · Feb`). `package.json` 1.22.0 → 1.22.1.

- **2026-09-20** — **Featured Travel Vibes rebuilt off the real Vibe Engine.** The home-page section was a `collection_grid` over the ten generic `/travel-vibes/` marketing leaves, all sharing one stock `hero.jpg` — nothing behind it. It is now a new `vibe_cards` section (`src/_includes/sections/vibe_cards.njk`) rendering a web port of the app's **Destination Result card**: 240px photo, white "Vibe Score" stamp top-left, dashed azure gateway roundel top-right, flat-azure drop-cap name banner, amber-ruled reason line, azure-over-amber button stack (`.dest-*` CSS in `styles.css`). Six picks, each a top-tier vibe matched to the destination nobody names first — Oaxaca (not Cancún), San Sebastián (not Paris), Crete/Heraklion (not Santorini), Quebec City (not Montreal), Kanazawa (not Kyoto), Chiang Mai (not Bangkok). Every card cycles that destination's **own** stock photos (crossfade, staggered per card, no dots) and links to a new `/featured-vibes/<slug>/` page carrying the full sub-region vibe blog, the engine's note, every other vibe that destination ranks for, and the photo gallery with credits (`src/featured-vibes.njk`). Data comes from a committed `src/_data/featuredVibes.json` generated by `node scripts/gen-featured-vibes.js` (Supabase REST, anon key) — deliberately **not** a build-time fetch, so a deploy can't fail or silently change the home page on a network blip or a re-ranking pass. Re-run the script and commit the JSON when the picks or the engine change. The card's Vibe Score is a fixed 91–97 marketing number (`displayScore`); the engine's raw score rides along as `engineScore` and is what the detail page's "every vibe" list shows. `vibe_cards` registered in the CMS section palette. `package.json` 1.21.0 → 1.22.0. Build green (86 files, all six pages in sitemap.xml via `addAllPagesToCollections`).

- **2026-06-27** — **Affiliate infra Step 2: web accounts + referral attribution.** Added real Supabase-backed user accounts to the site (same user base as the app): `/account/signup`, `/account/login`, `/account/reset` (request + set-new from recovery link), `/account/profile` (gated; app↔web parity incl. `flight_prefs`), `/account/` router. Supabase client config in `src/_data/supabase.json` (PUBLIC anon key only — RLS-gated; **never** add the service-role key) injected via `base.njk` → `window.WT_SUPABASE`; logic in `src/assets/js/wt-auth.js` (supabase-js from esm.sh) + `src/assets/account.css` + `layouts/account.njk`. Affiliate-link capture `/affiliate/<code>`: served via the **404.html catch-all** (`src/404.njk`) + `/affiliate/` index (`src/affiliate.njk`) + `src/assets/js/wt-affiliate.js` → sets 60-day `wt_ref` cookie + calls the `track-click` edge fn → redirects to signup. Account pages excluded from sitemap + already `Disallow`-ed in robots; **not** linked in public nav yet (pre-launch). `package.json` 1.1.0 → 1.2.0. Build green (45 files). Backend pieces (in the app repo): `track-click` edge fn + `affiliate_attribution_trigger.sql`. Full design: `D:\_Apps\_Flights\Wander_App\Affiliate_Infrastructure.md`.
...
- **2026-06-25** — Mobile hero reorder (≤960px): was image-on-top (`order:-1`); now **eyebrow → h1 → image → lede → buttons** via `display:contents` on `.hero-copy` + flex `order`. Desktop two-column + image badge unchanged. Applied to hero section + vibe/destination leaf heroes.
- **2026-06-25** — Mobile fix: added a **hamburger menu** (≤720px). The desktop nav was hidden with no replacement; now a toggle opens a full-width panel with the primary nav + partner links + launch-aware CTA (animated to an X, closes on tap). `header.njk` + `styles.css` + `base.njk` script. Deployed.
